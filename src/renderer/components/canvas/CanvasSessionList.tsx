...
export function CanvasSessionList(): JSX.Element | null {
  ...
  return (
    <aside
      className={cn(
        'canvas-session-list z-20 flex overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-primary)]/92 shadow-xl backdrop-blur transition-all duration-150',
        collapsed
          ? 'canvas-session-list--collapsed h-9 w-fit flex-none flex-row'
          : isVertical
            ? 'canvas-session-list--vertical h-[min(440px,calc(100vh_-_128px))] w-56 flex-col'
            : 'canvas-session-list--horizontal h-12 flex-row',
      )}
    >
      <div
        className={cn(
          'flex shrink-0 items-center border-[var(--color-border)]',
          collapsed ? 'h-full w-fit' : isVertical ? 'h-9 w-full border-b' : 'h-full border-r',
        )}
      >
        <button
          ...
          className={cn(
            'flex h-full min-w-0 items-center text-left transition-colors hover:bg-[var(--color-bg-hover)]',
            collapsed ? 'w-fit flex-none gap-2 px-2.5' : 'flex-1 justify-between px-3',
          )}
          title={collapsed ? '展开画布会话' : '折叠画布会话'}
        >
          <span className="flex min-w-0 items-center gap-2">
            {!collapsed && <CollapseIcon size={13} className="shrink-0 text-[var(--color-text-tertiary)]" />}
            <span className="truncate text-[var(--ui-font-xs)] font-semibold text-[var(--color-text-secondary)]">Cards</span>
          </span>
          <span className={cn(
            'rounded-full bg-[var(--color-bg-surface)] px-2 py-0.5 text-[var(--ui-font-2xs)] text-[var(--color-text-tertiary)]',
            !collapsed && 'ml-2',
          )}>
            {items.length}
          </span>
      ...
      {!collapsed && (
        ...
        >
          {items.length === 0 ? (
            <div className={cn(
              'flex items-center justify-center rounded-[var(--radius-md)] px-3 text-[var(--ui-font-xs)] text-[var(--color-text-tertiary)]',
              isVertical ? 'min-h-16 w-full' : 'h-full min-w-32',
            )}>
              暂无卡片
            </div>
