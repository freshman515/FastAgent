  ...
  return (
    <div className="flex flex-col gap-2 p-3">
      ...
      <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1.5 items-center">
        <span className="text-[var(--ui-font-2xs)] text-[var(--color-text-tertiary)]">Name</span>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="My App" className={INPUT} autoFocus />

        <span className="text-[var(--ui-font-2xs)] text-[var(--color-text-tertiary)]">Command</span>
        <input value={command} onChange={(e) => setCommand(e.target.value)} placeholder="dotnet / npm / python" className={INPUT} />

        <span className="text-[var(--ui-font-2xs)] text-[var(--color-text-tertiary)]">Args</span>
        <input value={args} onChange={(e) => setArgs(e.target.value)} placeholder="run --project ./MyApp" className={INPUT} />

        <span className="text-[var(--ui-font-2xs)] text-[var(--color-text-tertiary)]">CWD</span>
        <input value={cwd} onChange={(e) => setCwd(e.target.value)} placeholder="(project root)" className={INPUT} />

        <span className="text-[var(--ui-font-2xs)] text-[var(--color-text-tertiary)]">Env</span>
        <textarea value={env} onChange={(e) => setEnv(e.target.value)} placeholder="KEY=VALUE (one per line)" rows={2} className={cn(INPUT, 'resize-none')} />
      ...
      <div className="flex justify-end gap-1.5 pt-1">
        <button onClick={onCancel} className="px-2 py-1 text-[var(--ui-font-xs)] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)]">Cancel</button>
        <button
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--color-border)]">
          <span className="text-[var(--ui-font-sm)] font-semibold text-[var(--color-text-primary)]">
            {mode === 'add' ? 'New Launch Profile' : mode === 'edit' ? 'Edit Profile' : 'Run'}
          </span>
          {mode === 'list' && (
            <button
              onClick={() => setMode('add')}
          ...
          <div className="max-h-[300px] overflow-y-auto">
            {profiles.length === 0 ? (
              <div className="p-4 text-center">
                <p className="text-[var(--ui-font-xs)] text-[var(--color-text-tertiary)] mb-2">No launch profiles configured</p>
                <button
                  onClick={() => setMode('add')}
              ...
              profiles.map((p) => (
                <div key={p.id} className="group flex items-center gap-2 px-3 py-2 hover:bg-[var(--color-bg-surface)] transition-colors">
                  ...
                  <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleRun(p)} className="p-1 rounded text-[var(--color-success)] hover:bg-[var(--color-bg-tertiary)]" title="Run">
                      <Play size={12} fill="currentColor" />
                    </button>
                    <button onClick={() => { setEditingId(p.id); setMode('edit') }} className="p-1 rounded text-[var(--color-text-tertiary)] hover:bg-[var(--color-bg-tertiary)]" title="Edit">
                      <Edit3 size={12} />
                    </button>
                    <button onClick={() => removeProfile(p.id)} className="p-1 rounded text-[var(--color-error)] hover:bg-[var(--color-bg-tertiary)]" title="Delete">
                      <Trash2 size={12} />
                    </button>
                  </div>
                </div>
